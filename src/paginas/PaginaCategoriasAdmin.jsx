  ...
  return <MarcoTorneo tituloMovil="Administración" subtituloMovil="Categorías">
    <div className="admin-preguntas">
      ...
      {!cargando && !error && (rol === 'ADMINISTRADOR' || vistaPrevia) && <section className="admin-preguntas__panel" aria-labelledby="titulo-categorias-admin">
        ...
        {visibles.length ? <div className="admin-categorias__tabla"><div className="admin-categorias__cabecera" aria-hidden="true"><span>Categoría</span><span>Preguntas</span><span>Estado</span></div><ul>{visibles.map((categoria) => <li key={categoria.id}><strong>{categoria.nombre}</strong><span className="admin-categorias__cantidad">{categoria.cantidadPreguntas}<span className="admin-categorias__unidad"> preguntas</span></span><span className={`admin-categorias__estado${categoria.estado === 'BORRADOR' ? ' admin-categorias__estado--borrador' : ''}`}>{categoria.estado}</span></li>)}</ul></div> : <p className="admin-preguntas__vacio">No hay categorías que coincidan con la búsqueda.</p>}
      </section>}
    </div>
  </MarcoTorneo>
}
